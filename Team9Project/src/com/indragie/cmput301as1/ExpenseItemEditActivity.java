...
public class ExpenseItemEditActivity extends ExpenseItemAddActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// The superclass shows a custom action bar with Cancel and Done buttons
		// by default
		...
		ExpenseItem item = (ExpenseItem) getIntent().getSerializableExtra(
				EXTRA_EXPENSE_ITEM);
		editable = intent.getBooleanExtra(EXTRA_EXPENSE_ITEM_EDITABLE, false);
		expenseLocation = item.getLocation();
	 ...
	 *            The expense item.
	 * @param editable
	 *            Boolean if fields are editable.
	 */
	private void setupFields(ExpenseItem item, Boolean editable) {
		...
		if (expenseLocation != null) {
			locationField.setText(item.getLocation().toString());
			locationField.setEnabled(editable);
		}

		SpinnerUtils.setSelectedItem(currencySpinner, amount.getCurrencyUnit()
				.toString());
	...
	protected Intent getResultIntent() {
		...
		intent.putExtra(EXTRA_EXPENSE_ITEM_POSITION,
				getIntent().getIntExtra(EXTRA_EXPENSE_ITEM_POSITION, -1));
		return intent;
	}
}
